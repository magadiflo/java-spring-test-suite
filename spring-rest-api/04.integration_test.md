...
En el archivo `02.unit_test.md` documentamos pruebas unitarias al controlador `AccountController` usando `MockMvc`.
En ese contexto:

- Simulábamos las llamadas HTTP sin levantar un servidor real.
- Usábamos `@MockBean` para simular el `AccountService`.
- Verificábamos respuestas simuladas con ayuda de `Mockito`.

### 🔁 ¿Qué cambia en pruebas de integración?

En esta sección reutilizamos `MockMvc`, pero ahora para realizar `pruebas de integración reales`:

- Se ejecuta el flujo completo: desde el endpoint HTTP hasta la capa de persistencia.
- No se usan mocks ni simulaciones: se accede a datos reales en la base de datos.
- Se valida el comportamiento del controlador en condiciones similares a producción.

La siguiente clase configura la base para nuestras pruebas. Usamos el mismo esquema que en las pruebas anteriores,
manteniendo el perfil `test`, la carga de datos controlada con `@Sql`, y el entorno web aleatorio para evitar
conflictos de puerto.

### 📌 Nota técnica sobre `@AutoConfigureMockMvc`

En pruebas unitarias, `MockMvc` se configura automáticamente con `@WebMvcTest`. En pruebas de integración,
`necesitamos agregar explícitamente` `@AutoConfigureMockMvc` para que `Spring Boot` registre el bean `MockMvc`
en el contexto completo iniciado por `@SpringBootTest`.

### 🧠 ¿Por qué usar `MockMvc` en integración?

- Permite validar el comportamiento del controlador sin depender de un cliente externo (`WebTestClient`,
  `RestTemplate`).
- Ofrece una API fluida (`perform`, `andExpect`) para verificar estado, headers y cuerpo.
- Es ideal para proyectos `Spring MVC` que no requieren stack reactivo.

### ⚙️ Configuración inicial de la clase de prueba

La siguiente clase mantiene la misma estructura que las pruebas previas. La diferencia clave aquí es la inclusión de la
anotación `@AutoConfigureMockMvc`, que **inicializa y configura automáticamente una instancia de** `MockMvc`
dentro del contexto de Spring Boot.

Esto nos permite inyectarla con `@Autowired` y ejecutar solicitudes HTTP reales (GET, POST, etc.) contra los endpoints
del controlador, sin necesidad de levantar manualmente un servidor externo.
...
💡 Nota
> La anotación `@AutoConfigureMockMvc` es esencial cuando usamos `@SpringBootTest`, ya que por defecto este no expone
> un bean de tipo `MockMvc`. Sin esta anotación, la inyección con `@Autowired` fallaría.

### 🧩 Implementación de la prueba de integración del AccountController con MockMvc

Esta clase replica los mismos casos de prueba que las versiones con `WebTestClient` y `TestRestTemplate`, pero
utilizando `MockMvc`. A diferencia de las pruebas unitarias, aquí se ejecuta el flujo completo desde el controlador
hasta la base de datos, `sin simulaciones`.

💡 Nota técnica
> En este contexto, `MockMvc` ya no actúa como un “simulador de requests”, sino como un cliente HTTP completamente
> integrado al contexto de Spring Boot, gracias a la combinación de `@SpringBootTest` y `@AutoConfigureMockMvc`.
...
## 🚀 Ejecución de todas las pruebas del proyecto (unitarias e integración)

Habiendo completado la implementación de todas las pruebas unitarias e integración de nuestro proyecto, es momento de
ejecutar el conjunto completo para verificar el comportamiento general y la correcta interacción entre los distintos
componentes.

Podemos realizar la ejecución desde nuestro IDE (`IntelliJ IDEA`) o desde la línea de comandos (`Maven`).
Veamos ambas opciones:

### 🖥️ Ejecución en IntelliJ IDEA

Podemos ejecutar todos los tests del proyecto directamente desde `IntelliJ IDEA`:

- El botón `Run All Tests` del proyecto
- O clic derecho sobre el paquete `src/test/java` → `Run Tests in...`

![07.png](assets/07.png)

💡 Nota: `IntelliJ IDEA` usa su propio motor de ejecución optimizado y paralelo para `JUnit`, por lo que los tiempos
suelen ser considerablemente menores que en la ejecución por consola.

### ⚙️ Ejecución desde la consola con Maven

Desde la raíz del proyecto `spring-rest-api`, ejecutamos el siguiente comando `mvn test`. Esto instruye a `Maven`
para compilar el código fuente, compilar las pruebas y luego ejecutar todas las clases de prueba encontradas bajo
`src/test/java`.
...
[INFO] --------------------------------[ jar ]---------------------------------
[INFO]
[INFO] --- resources:3.3.1:resources (default-resources) @ spring-rest-api ---
[INFO] Copying 4 resources from src\main\resources to target\classes
...
2025-10-22T16:07:58.708-05:00 DEBUG 6028 --- [spring-rest-api] [           main] d.m.app.service.impl.AccountServiceImpl  : Iniciando transferencia | Origen: 1 | Destino: 2 | Monto: 5000
2025-10-22T16:07:58.708-05:00  INFO 6028 --- [spring-rest-api] [           main] d.m.app.service.impl.AccountServiceImpl  : Validando el retiro de saldo para la cuenta con ID: 1, saldo actual: 2000, monto a retirar: 5000
2025-10-22T16:07:58.708-05:00  WARN 6028 --- [spring-rest-api] [           main] d.m.app.service.impl.AccountServiceImpl  : Solicitud rechazada por saldo insuficiente | Cuenta ID: 1 | Titular: Milagros | Saldo: 2000 | Monto solicitado: 5000
2025-10-22T16:07:58.718-05:00 DEBUG 6028 --- [spring-rest-api] [           main] .c.s.DirtiesContextTestExecutionListener : After test method: class [AccountServiceImplSpringBootTest], method [shouldThrowInsufficientBalanceExceptionWhenSourceAccountHasLowBalance], class annotated with @DirtiesContext [false] with mode [null], method annotated with @DirtiesContext [false] with mode [null]
2025-10-22T16:07:58.718-05:00 DEBUG 6028 --- [spring-rest-api] [           main] o.s.t.c.w.ServletTestExecutionListener   : Resetting RequestContextHolder for test class dev.magadiflo.app.unit.service.AccountServiceImplSpringBootTest
2025-10-22T16:07:58.718-05:00 DEBUG 6028 --- [spring-rest-api] [           main] .c.s.DirtiesContextTestExecutionListener : After test class: class [AccountServiceImplSpringBootTest], class annotated with @DirtiesContext [false] with mode [null]
[INFO] Tests run: 11, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 0.167 s -- in dev.magadiflo.app.unit.service.AccountServiceImplSpringBootTest
[INFO]
[INFO] Results:
[INFO]
...
[INFO] Finished at: 2025-10-22T16:07:59-05:00
[INFO] -----------------------------------------------------------------------
````

### 🧠 Análisis de resultados

El resumen final indica que se ejecutaron `76 pruebas sin fallos` (Failures: 0, Errors: 0), lo cual significa que
todas las capas —servicios, repositorios y controladores— funcionan correctamente en conjunto bajo el perfil `test`.

### 💡 Diferencia de tiempos

En consola la ejecución tomó `~27 segundos`, mientras que en `IntelliJ IDEA` fue de `~6 segundos`.
Esto ocurre porque el IDE mantiene `caché de clases compiladas` y optimiza la carga del `ApplicationContext de Spring`,
mientras que `Maven` realiza una compilación completa en cada ejecución.

## 🏷️ Excluir clases de test mediante @Tag

Este tema ya lo habíamos mencionado en el `README` del proyecto `junit5-test`, donde aprendimos a incluir tests
específicos usando su etiqueta (`@Tag`).

En esta lección, veremos el proceso `inverso`: cómo `excluir` determinados tests durante la ejecución, tanto desde
`IntelliJ IDEA` como desde la `línea de comandos con Maven`.

### 💡 ¿Por qué excluir tests?

En algunos casos, puede que queramos `ejecutar todos los tests excepto algunos`. Por ejemplo:

- Queremos omitir los tests de integración que son más lentos.
- Deseamos excluir tests que dependen de recursos externos (como una base de datos o API remota).
- O simplemente queremos enfocar la ejecución en un subconjunto temporal.

Para esos escenarios, los `tags de JUnit 5` (`@Tag`) resultan especialmente útiles, ya que nos permiten
`incluir o excluir grupos de pruebas` sin tener que modificar el código ni comentar clases.

### ⚙️ Ejemplo práctico

A continuación, se muestra la clase de prueba de integración `AccountControllerTestRestTemplateTest`, a la cual hemos
asignado la etiqueta `@Tag("integration3")`.

Esto nos permitirá filtrar su ejecución según el entorno o configuración del proyecto.
...
### 🖥️ Excluyendo un tag en IntelliJ IDEA
...
2. En la sección `Build and run`:
    ...
    - Cambiar a `Tags`.
3. En el campo de etiquetas, escribir el tag que queremos excluir usando el signo de admiración (`!`), por ejemplo:
   `!integration3`.

![08.png](assets/08.png)

> 💡 Nota: El signo `!` delante del nombre del tag indica a `JUnit` que debe `excluir todas las clases o métodos`
> marcados con esa etiqueta.

### 🧭 Resultado de la ejecución

Al ejecutar nuevamente todas las pruebas del proyecto, observamos que:

- La clase `AccountControllerTestRestTemplateTest` ya no se ejecuta.
- El número total de pruebas disminuye de `76` a `69`.
- Todas las pruebas restantes se ejecutan correctamente.

![09.png](assets/09.png)

### 💻 Excluyendo un tag desde la línea de comandos (Maven)

También podemos lograr el mismo comportamiento desde consola con el siguiente comando:
`mvn test -Dgroups=!integration3`.
...
[INFO] --------------------------------[ jar ]---------------------------------
...
2025-10-22T17:07:08.977-05:00 DEBUG 21412 --- [spring-rest-api] [           main] o.s.t.c.w.ServletTestExecutionListener   : Resetting RequestContextHolder for test class dev.magadiflo.app.unit.service.AccountServiceImplSpringBootTest
2025-10-22T17:07:08.977-05:00 DEBUG 21412 --- [spring-rest-api] [           main] .c.s.DirtiesContextTestExecutionListener : After test class: class [AccountServiceImplSpringBootTest], class annotated with @DirtiesContext [false] with mode [null]
[INFO] Tests run: 11, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 0.164 s -- in dev.magadiflo.app.unit.service.AccountServiceImplSpringBootTest
[INFO]
[INFO] Results:
[INFO]
...
[INFO] Finished at: 2025-10-22T17:07:09-05:00
[INFO] ------------------------------------------------------------------------
````

Como se observa, `Maven` ejecuta `69 pruebas` exitosamente, excluyendo todas las asociadas al tag `integration3`,
de la misma manera que `IntelliJ IDEA`.

### 📌 Comparativa de uso de `@Tag`

| Acción                      | Sintaxis               | Resultado                                                       |
|:----------------------------|------------------------|-----------------------------------------------------------------|
| Ejecutar solo tests con tag | `groups=account`       | Ejecuta solo los tests con `@Tag("account")`                    |
| Excluir tests con tag       | `groups=!integration3` | Ejecuta todos los tests excepto los marcados con `integration3` |

Ambas sintaxis funcionan tanto en `IntelliJ IDEA` (desde la configuración de ejecución) como en línea de comandos con
`Maven`, lo que nos permite filtrar fácilmente qué pruebas ejecutar según el contexto o entorno.
