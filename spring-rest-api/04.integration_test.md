...
## ⚙️ Configuración base de pruebas (`application.yml`)
...
logging:
  level:
    ...
    org.hibernate.type.descriptor.sql.BasicBinder: TRACE  # Muestra parámetros enviados al SQL
...
Cuando usamos la siguiente configuración en `src/main/resources/application.yml` (desarrollo/producción):
...
Esto dice: `Cuando arranques la aplicación, usa el perfil dev automáticamente`. Por lo que, al ejecutar:

````bash
$ java -jar app.jar
````

Sin especificar nada, usará `dev` porque está configurado como activo por defecto. Si queremos cambiarlo, basta
...
Cuando usamos la siguiente configuración en `src/test/resources/application-test-h2.yml` (test, test-h2, etc):
...
"Este archivo `application-test-h2.yml` se aplicará cuando el perfil `test-h2` esté activo. Es una declaración
condicional, no una activación.

#### 📢 Importante

> En archivos que siguen la convención `application-{profile}.yml`, como `application-test-h2.yml`, esta propiedad
> es `opcional`. Spring Boot ya sabe que debe aplicar ese archivo si el perfil `test-h2` está activo
> (por ejemplo, con `@ActiveProfiles("test-h2")` en nuestros tests). Se recomienda usarla solo si necesitamos
> mayor control o si el archivo contiene múltiples bloques para distintos perfiles.
...
| `spring.config.activate.on-profile` | `DECLARA` cuándo aplicar este archivo | Archivos específicos de perfil |

### 🔧 ¿Cómo se activan los perfiles en tests entonces?

Hay 3 formas:

- `Opción 1`: Con `@ActiveProfiles` (la más común en tests)
    ````java
    @SpringBootTest
    ...
    ````
  Es la forma más usada y clara. Cada clase define explícitamente su perfil.
...
- `Opción 2`: Con `spring.profiles.active` en `application.yml` de `/test`
    ...
    ````
  Si ponemos esto, `todos` los tests usarán `test-h2` a menos que lo sobrescribamos con `@ActiveProfiles`.


- `Opción 3`: Con Maven/Gradle al ejecutar
    ````bash
...
cada clase de test. ¿Cuáles son sus ventajas?

- `Flexibilidad`: Cada clase de test puede elegir su perfil con `@ActiveProfiles`.
- `Claridad`: Al leer el test, ves inmediatamente qué perfil usa.
...
2. Cada clase de test con `@ActiveProfiles("test-h2")` carga:
    ...
    - `application-test-h2.yml` (porque el perfil `test-h2` está activo)
3. Los tests sin `@ActiveProfiles` solo cargan `application.yml` (base).

> 🎯 No necesitamos especificar el perfil al ejecutar `mvn test` si ya lo declaramos con `@ActiveProfiles`.
...
### 🎯 Alcance de las pruebas con `@DataJpaTest`

Al usar esta anotación, Spring Boot levanta un `contexto de aplicación parcial`, cargando únicamente los componentes
necesarios para probar la capa de persistencia:

- Entidades (`@Entity`)
- Repositorios (`@Repository`)
- Configuración de `JPA/Hibernate`
- Base de datos embebida (por defecto, `H2`)

Esto hace que las pruebas sean `ligeras`, `rápidas` y `enfocadas`, pero aun así reales, ya que interactúan con una base
de datos y con el `EntityManager`.
