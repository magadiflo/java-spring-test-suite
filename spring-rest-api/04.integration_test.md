...
### Opción 1: Por nombre de clase (`-Dtest=...`)
...
### Opción 2: Por etiquetas (@Tag)

`JUnit 5` permite usar la anotación `@Tag` para clasificar tests de forma lógica, independientemente del nombre de la
clase.
...
- Permite agrupar varias clases bajo la misma categoría sin depender de su nombre.
- Puede combinarse con diferentes perfiles (`@ActiveProfiles`) según el tipo de test.
- Muy útil cuando una suite contiene muchos tipos de tests y se requiere ejecutar subconjuntos específicos.
